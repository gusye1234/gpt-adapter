...
def get_hidden_size(model):
    ...
    raise NotImplementedError("Only support huggingface model for now"
...
def normal_transit_fn(before_module : nn.Module, 
                      after_module : nn.Module):
    after_module = after_module.to(get_module_device(before_module))
    return after_module.load_state_dict(
        before_module.state_dict(), strict=False
    )
...
def locate_replace_module(model : nn.Module, 
    ...
    for key, before_module in model.named_modules():
        ...
        logging.info(f"Replace one {key}, seen {count_time} times, {before_module} -> {after_module}")
    return model